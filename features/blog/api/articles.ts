...
import type {
  ...
  SingleArticleQuery,
  SingleArticleQueryVariables,
  MappedPost,
} from '../types'
import { mapArticleToPost } from '../utils'
...
export const getAllArticles = cache(async (): Promise<MappedPost[]> => {
  ...
  try {
    ...
    if (errors) {
      console.error('[Blog] GraphQL errors:', errors)
      return []
    }
    ...
    console.error('[Blog] Failed to fetch articles:', error)
    return []
  }
})
...
export const getArticleById = cache(async (documentId: string): Promise<MappedPost | null> => {
  ...
  try {
    ...
    console.error('[Blog] Failed to fetch article:', error)
    return null
  }
})
